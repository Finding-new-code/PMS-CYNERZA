...
export default function Inventory() {
    ...
    const apiEnd = format(addDays(endDate, 1), 'yyyy-MM-dd'); // API expects exclusive end date or inclusive? backend doc says "end: End date (exclusive, check-out date)" for range availability.
    // wait, get_inventory doc says: "start: Start date (inclusive)", "end: End date (exclusive)".
    // So if asking for May 1 to May 31, I should pass May 1 and June 1.
    ...
    const previousMonth = () => setCurrentDate(curr => addDays(startOfMonth(curr), -1));
    const nextMonth = () => setCurrentDate(curr => addDays(endOfMonth(curr), 1));
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                        <tr>
                            <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase sticky left-0 bg-gray-50 z-10 w-48">
                                Date
                            </th>
                            {roomTypes?.filter(rt => selectedRoomType === 'all' || rt.id === Number(selectedRoomType)).map(rt => (
                                <th key={rt.id} className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase min-w-[120px]">
                                    {rt.name}<br />
                        ...
                        {daysInMonth.map(day => (
                            <tr key={day.toISOString()} className="hover:bg-gray-50">
                                ...
                                {roomTypes?.filter(rt => selectedRoomType === 'all' || rt.id === Number(selectedRoomType)).map(rt => {
                                    ...
                                    return (
                                        <td key={`${day}-${rt.id}`} className="px-4 py-3 text-center border-l border-gray-100">
                                            {data ? (
                                                <div className="flex flex-col items-center">
                                                    <span className={cn(
                                                        "text-sm font-bold",
                                                        data.available_rooms === 0 ? "text-red-600" :
                                                            data.available_rooms < 5 ? "text-orange-600" : "text-green-600"
                                                    )}>
